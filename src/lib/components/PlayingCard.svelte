<script lang="ts">
	...
	const cardColors = { heart: 'red', diamond: 'red', spade: 'black', club: 'black' };

	$: color = cardColors[cardSuit] ? (cardFlipped ? cardColors[cardSuit] : 'black') : 'black';
...
<div
	class="relative bg-gray-100 {cardRaised ? 'shadow-2xl' : 'shadow-sm'} rounded-sm select-none"
...
>
	{#if cardFlipped}
		<div class="grid grid-cols-2 place-content-between h-full w-full text-center">
			<div
			...
			{#if button}
				<div
					...
					on:click={cardHover && !cardRaised ? () => dispatch('group') : () => {}}
				>
					<Multiple />
				</div>
			{:else}
			...
			>
				<!-- Bottom Right -->
				<div class="rotate"><CorrectSuit {cardSuit} strokeWidth="2.5" /></div>
				{#if cardValue != 0}
					<div class="rotate">{cardNames[cardValue] ? cardNames[cardValue] : cardValue}</div>
				{:else}
					<div class="rotate-180">{cardNames[cardValue] ? cardNames[cardValue] : cardValue}</div>
				{/if}
			</div>
		</div>
		<!-- Decoration -->
		...
		{:else if cardValue == 11}
			<div class="absolute left-8 -rotate-45 text-xl font-bold" style="top: 4.3rem;">Jack</div>
		{:else if cardValue == 12}
			<div class="absolute left-6 -rotate-45 text-xl font-bold" style="top: 4.3rem;">Queen</div>
		{:else if cardValue == 13}
			<div class="absolute left-9 -rotate-45 text-xl font-bold" style="top: 4.3rem;">King</div>
		{:else if cardValue == 0}
			<div class="absolute left-1.5 text-xl font-bold animate-bounce" style="top: 3rem;">
				<Joker size={6.6} />
			</div>
		{:else if cardValue == 10}
			<div class="absolute left-5 -rotate-45 text-xl font-bold" style="top: 4.2rem;">
		...
		{:else}
			<div class="absolute left-7 -rotate-45 text-xl font-bold" style="top: 4.2rem;">
				<div class="flex flex-row">
					<div>
						{cardValue} x
					</div>
	...
	{:else}
		...
		<div class="absolute left-3 -rotate-45 text-xl font-bold" style="top: 4.3rem;">Cardback</div>
	{/if}
</div>
